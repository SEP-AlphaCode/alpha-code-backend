...
async def load_all_durations():
    """Load toàn bộ durations từ DB."""
    global DANCE_DURATIONS_MS, ACTION_DURATIONS_MS, EXPRESSION_DURATIONS_MS
    DANCE_DURATIONS_MS = await load_dance_durations()
    ACTION_DURATIONS_MS = await load_action_durations()
    EXPRESSION_DURATIONS_MS = await load_expression_durations()
    
def get_durations_summary() -> dict:
    """Trả về snapshot durations hiện tại để debug/test."""
    return {
        "dance": DANCE_DURATIONS_MS,
        "action": ACTION_DURATIONS_MS,
        "expression": EXPRESSION_DURATIONS_MS,
    }
